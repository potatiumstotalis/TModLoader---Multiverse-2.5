...
using MultiverseRel.Content.Subworlds;
...
namespace MultiverseRel.Content.Tiles
{
    ...
    public class PortalUI : UIState
    {
        ...
        public override void OnInitialize()
        {
            ...
            if (ModContent.GetContent<Subworld>().Any())
            {
                ...
                {
                    // Handle the case where _picker is null
                    // For example, log an error or add an error message to the UI
                    list.Add(new UITextPanel<string>("ERROR: Subworld TP Filters are not configured."));
                }
            }
            else
            {
                list.Add(new UITextPanel<string>(
                    "ERROR: No Subworlds Found. \nEither add one in the config, or install a mod that adds one. Always remember to set up your TP Filters when adding subworlds in the config")
                {
                    Width =
                    {
                        Precent = 1
            ...
            var saveButton = new UIIconTextButton(Language.GetText("Mods.MultiverseRel.Save"), Color.White,
            ...
            saveButton.OnLeftClick += delegate { Save(); };
            list.Add(saveButton);
        }
    }
}